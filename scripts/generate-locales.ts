...
const EXT_NAME = 'Modrinth Extras'
...
function loadSummary(locale: string): { summary?: string; empty: boolean } {
	...
	try {
		...
		if (Object.keys(local).length === 0) return { empty: true }
		return { summary: local['meta.summary']?.defaultMessage, empty: false }
	} catch {
		return { empty: true }
	}
}
...
for (const locale of localeDirs) {
	...
	const { summary, empty } = loadSummary(locale)
	if (!summary) {
		if (!empty) console.warn(`No meta.summary for locale "${locale}", skipping.`)
		continue
	}
